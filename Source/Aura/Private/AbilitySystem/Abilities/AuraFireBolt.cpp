...
FString UAuraFireBolt::GetDescription(int32 Level)
{
	...
	if (Level == 1)
	{
		ExplainNumOfFireBolts = FString::Printf(TEXT("<Default>Launches a bolt of fire, "
			"exploding on impact and dealing:</>\n"));
	}
	else
	{
		ExplainNumOfFireBolts = FString::Printf(TEXT("<Default>Launches %d bolt of fire, "
                                     ...
                                     bool bOverridePitch, float PitchOverride, AActor* HomingTarget)
{
	...
	for (const FRotator& Rot : Rotations)
	{
		...
		if (bLaunchHomingProjectiles && IsValid(HomingTarget))
		{
			...
			Projectile->ProjectileMovement->HomingAccelerationMagnitude = FMath::RandRange(
